...
function initFirebase(){
  ...
  // document.getElementById('login-screen').classList.remove('hidden'); // deshabilitado — acceso desde cuenta admin de la web
  const scripts = [
  ...
  try{
    ...
    firebase.auth().onAuthStateChanged(user=>{
      if(user){
        ...
        // showLogin(); // deshabilitado — acceso directo sin login gate
      }
    });
  } catch(e){
...
function renderPortalUsers() {
  ...
  const rows = filtered.map(([uid, u]) => {
    ...
    const rolBadge = p.rol === 'admin'
      ? `<span style="background:rgba(249,115,22,0.15);color:var(--ferrari);border:1px solid rgba(249,115,22,0.3);padding:2px 7px;border-radius:10px;font-size:10px;font-weight:700">ADMIN</span>`
      : `<span style="background:rgba(255,255,255,0.05);color:var(--text3);padding:2px 7px;border-radius:10px;font-size:10px">USER</span>`;
      ...
      <table style="width:100%;border-collapse:collapse;font-size:13px">
        <thead>
          <tr style="border-bottom:1px solid rgba(255,255,255,0.08)">
            ...
            <th style="padding:8px 12px;text-align:left;font-size:10px;color:var(--text3);font-family:var(--mono);font-weight:600;text-transform:uppercase;letter-spacing:1px">Rol</th>
...
function openPortalUserDetail(uid) {
  ...
  // Rol — solo editable por el admin principal
  const currentUserEmail = (typeof firebase !== 'undefined' && firebase.apps.length && firebase.auth().currentUser)
    ? firebase.auth().currentUser.email : '';
  const isMainAdmin = currentUserEmail === 'wmarcano.business@gmail.com';
  const rolSel = document.getElementById('portal-edit-rol');
  rolSel.value = p.rol || 'user';
  rolSel.disabled = !isMainAdmin;
  const rolNote = document.getElementById('portal-role-note');
  if (rolNote) rolNote.style.display = isMainAdmin ? 'none' : 'block';
...
function savePortalUserConfig() {
  ...
  // Rol — solo lo guarda el admin principal
  const currentEmail = firebase.auth().currentUser?.email || '';
  if (currentEmail === 'wmarcano.business@gmail.com') {
    updates.rol = document.getElementById('portal-edit-rol').value || 'user';
  }
